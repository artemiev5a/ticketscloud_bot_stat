...
Команда `/stats` запрашивает все страницы `GET https://ticketscloud.com/v2/resources/orders`. Продажи рассчитываются по `values.full`, полностью возвращённые заказы не считаются успешными, а возвраты выводятся отдельно. Период определяется по `done_at` в часовом поясе `Europe/Moscow`; его можно изменить переменной `REPORT_TIME_ZONE`.
