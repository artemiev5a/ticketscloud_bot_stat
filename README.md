...
`ENCRYPTION_SECRET` и `CACHE_KEY_SECRET` должны быть разными случайными значениями длиной не менее 32 байт. Приложение намеренно не запускается без них и никогда не печатает секреты в логи.
